...
def msTPFP(line_pred, line_gt, threshold):
    ...
    dist = np.min(diff, 1)
    hit = np.zeros(len(line_gt), dtype=bool)  # 修复: np.bool -> bool
    tp = np.zeros(len(line_pred), dtype=np.float64)  # 修复: np.float -> float
    fp = np.zeros(len(line_pred), dtype=np.float64)  # 修复: np.float -> float
...
def TPFP(lines_dt, lines_gt, threshold):
    ...
    choice = np.argmin(diff,1)
    dist = np.min(diff,1)
    hit = np.zeros(len(lines_gt), dtype=bool)  # 修复: np.bool -> bool
    tp = np.zeros(len(lines_dt), dtype=np.float64)  # 修复: np.float -> float
    fp = np.zeros(len(lines_dt), dtype=np.float64)  # 修复: np.float -> float
