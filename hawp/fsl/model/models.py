...
class WireframeDetector(nn.Module):
    ...
    def forward(self, images, annotations=None, targets=None, return_debug=False):
        if self.training:
            return self.forward_train(images, annotations=annotations, return_debug=return_debug)
        else:
            return self.forward_test(images, annotations=annotations, return_debug=return_debug)
    ...
    def forward_test(self, images, annotations=None, return_debug=False):
        device = images.device
        ...
        # 【关键】准备所有需要传递给backbone的模块和函数
        external_modules = {
            'fc1': self.fc1,
            'fc3': self.fc3, 
            'fc4': self.fc4,
            'fc2': self.fc2,
            'fc2_head': self.fc2_head,
            'fc2_res': self.fc2_res,
            'wireframe_matcher': self.wireframe_matcher,
            'bilinear_sampling': self.bilinear_sampling,
            'compute_loi_features': self.compute_loi_features,
            'loi_cls_type': self.loi_cls_type,
        }
        
        # 【关键】传递所有模块给backbone，让它内部完成质量评估
        if return_debug:
            backbone_outputs = self.backbone(
                images, return_debug=True, 
                external_modules=external_modules
            )
            if len(backbone_outputs) == 3:
                outputs, features, backbone_debug = backbone_outputs
            else:
                outputs, features = backbone_outputs
                backbone_debug = None
        else:
            outputs, features = self.backbone(
                images, 
                external_modules=external_modules
            )
        ...
        juncs_pred, juncs_score = get_junctions(non_maximum_suppression(jloc_pred[0]),joff_pred[0], topk=topK,th=self.jhm_threshold)
        ...
        if return_debug:
            ...
            debug_info = {
                ...
                'raw_output': {
                    'md_pred': md_pred[0].cpu(),
                    'dis_pred': dis_pred[0].cpu(), 
                    'jloc_pred': jloc_pred[0].cpu(),
                    'joff_pred': joff_pred[0].cpu(),
                },
                'intermediate_results': {
                    'lines_pred_raw': lines_pred.cpu(),      # 原始线段proposals [N, 4] 特征图坐标
                    'juncs_pred_raw': juncs_pred.cpu(),      # 原始关键点predictions [N, 2] 特征图坐标
                    'lines_adjusted': lines_adjusted.cpu(),  # 匹配后的线段 [N, 4] 特征图坐标
                    'lines_scores': scores.cpu(),            # 线段分数 [N]
                    'feature_size': (output.size(2), output.size(3)),  # 特征图尺寸 (H, W)
                },
                'scale_factors': {
                    'sx': sx,  # width缩放因子
                    'sy': sy,  # height缩放因子
            ...
            return output, extra_info, debug_info
    ...
    def refinement_train(self, lines_batch, jloc_pred, joff_pred, loi_features, loi_features_thin, loi_features_aux, metas):
        ...
        # Add safety checks for empty tensors
            if lines_pred.shape[0] == 0 or lines_gt.shape[0] == 0:
                # Skip this batch if no predictions or ground truth
                extra_info['recall_hafm-05'] += 0.0
                extra_info['recall_hafm-10'] += 0.0
                extra_info['recall_hafm-15'] += 0.0
                
                # Create dummy loss for line features
                lines_pred_feat = torch.zeros((0, self.dim_edge_feature * (self.n_pts0-2)), device=device)
                lines_pred_logits = torch.zeros((0,), device=device)
                lines_pred_labels = torch.zeros((0,), device=device)
                
                loss_dict['loss_lineness'] += 0.0
            ...
            lines_pred_feat = self.compute_loi_features(loi_features_aux[i],lines_pred.detach())# 注意这里梯度断开了！！！因为这里只打算训练线段筛选器，不打算回传到主模型
            ...
            if lines_adjusted.shape[0] == 0 or lines_gt.shape[0] == 0:
                # Skip this batch if no predictions or ground truth
                extra_info['recall_hafm-05'] += 0.0
                extra_info['recall_hafm-10'] += 0.0
                extra_info['recall_hafm-15'] += 0.0
    ...
    def forward_train(self, images, annotations=None, return_debug=False):
        device = images.device
        targets, metas = self.hafm_encoder(annotations)
        self.train_step += 1
        
        # 准备所有需要传递给backbone的模块和函数
        external_modules = {
            'fc1': self.fc1,
            'fc3': self.fc3, 
            'fc4': self.fc4,
        ...
        if return_debug:
            ...
            debug_info = {
                'backbone_debug': backbone_debug,
                'annotations': annotations,  # 原始标注用于对比
                'metas': metas,              # 处理后的元数据
                'raw_output': {
                    'md_pred': md_pred[0].cpu(),
                    'dis_pred': dis_pred[0].cpu(), 
                ...
                'feature_size': (output.size(2), output.size(3)),  # 特征图尺寸
            }
            return loss_dict, extra_info, debug_info
